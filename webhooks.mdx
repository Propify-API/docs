...
curl -X POST \
  'https://<your-website.com>/webhook/leases' \
  --header 'Content-Type': 'application/json' \
  --header 'propify-signature': '<payload signature>', \
  --data-raw '{
      "meta": {
        "object": "leases",
        "integration_id": "1234567890",
      },
      "recordId: "cuid_1234567890",
      "diff": {
...
Propify signs every webhook payload with a signature. This signature is sent in the `propify-signature` header. You can use this signature to verify that the webhook was sent by Propify and not a malicious third party.
